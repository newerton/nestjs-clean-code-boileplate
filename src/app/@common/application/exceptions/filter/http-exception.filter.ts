...
export class HttpExceptionFilter implements ExceptionFilter {
  ...
  private handleNestError(
    ...
    if (error instanceof HttpException) {
      ...
      const errorErr = codeArray.find(
        (item) => item.code === error.getStatus(),
      );
      const response: string | Record<string, any> = error.getResponse();
      const errorMessage = response[Object.keys(response)[0]] || error.message;
      errorResponse = CoreApiResponse.error(
        error.getStatus() < 100 ? Code.INTERNAL_ERROR.code : error.getStatus(),
        errorErr?.error || Code.INTERNAL_ERROR.error,
        errorMessage,
        null,
      );
    }
